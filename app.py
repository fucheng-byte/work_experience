...
if prompt := st.chat_input("請輸入問題... (系統將分批閱讀，這可能需要 1~3 分鐘)"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            for i, batch_text in enumerate(st.session_state.text_batches):
                my_bar.progress((i) / total_batches, text=f"🔍 正在閱讀第 {i+1} / {total_batches} 批資料... (每批約需 10~20 秒，請稍候)")
                ...
                if "NO_MATCH" not in response.text.upper():
                    ...
                    live_output.warning(f"👀 報告：在第 {i+1} 批次中沒有找到相關資料，繼續往下翻...")
            ...
            my_bar.empty()
            st.error(f"系統分段讀取時發生錯誤，這可能是 Google 伺服器超載。請稍後再試。錯誤訊息: {str(e)}")
