...
def build_knowledge_base(data_dir, mod_time):
    ...
    for filename in pdf_files:
        ...
        for chunk in chunks:
            # 【關鍵修改】把檔名也加入到準備變成向量的文字中，讓檔名也能被搜尋到！
...
if "db_initialized" not in st.session_state:
    with st.spinner("🔄 正在初始化企業級知識庫（正在讀取並優化搜尋引擎，請耐心等候幾分鐘...）"):
...
if prompt := st.chat_input("請輸入關於出差經驗或客戶的問題... (例如: 幫我找張政乾的報告)"):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner(f"🔍 正在海量檔案中進行智慧檢索... (使用 {CHAT_MODEL})"):
            try:
                ...
                # 【關鍵修改】把調閱的資料量從 6 段擴大到 12 段，讓 AI 看得更廣
                top_k = scored_chunks[:12]
                ...
                if context_str:
                    full_prompt = (
                        ...
                        "在回答時，請務必主動提及你是從哪些來源檔案中找到這些資訊的。如果使用者是在找特定人名，請明確告訴他有哪些檔案符合。\n"
                    ...
                    full_prompt = (
                        ...
                        "請客氣地回答：'抱歉，目前的出差經驗知識庫中沒有檢索到直接相關的資料。您可以嘗試換個關鍵字，或確認人名是否輸入正確。'"
                    )
